...
def compare(user_score,computer_score):
    if user_score==computer_score:
        ...
        return "Win with balckjak"
    elif user_score>21:
        return "You went over. You loose"
    elif user_score>computre_score:
        return "You win"
    else:
        return "you loose"
...
while not is_game_over:
    ...
    computre_score=calculate_score(computer_cards)
    print(f"Your cards is {user_cards} and your score is: {users_score}")
    print(f"computer cards={computre_score}")
    if users_score==0 or computre_score==0 or users_score==21:
        is_game_over=True
    else:
        user=input("if they want to draw another card. type yes or no\n").lower()
        if user=='yes':
            user_cards.append(deal_card())
        else:
            is_game_over=True
...
while computre_score!=0 and computre_score < 17:
    computer_cards.append(deal_card())
    computre_score=calculate_score(computer_cards)

print(compare(users_score,computre_score))
